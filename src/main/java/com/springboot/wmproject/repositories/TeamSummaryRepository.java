...
public interface TeamSummaryRepository extends JpaRepository<TeamSummary,Integer> {
    String getSummaryTeamOrganization = """
                    ...
                    LIMIT 0 , 1000) AS Q1
                    LEFT JOIN
                    (SELECT e.team_id, o.team_name, MAX(e.name) AS name, MAX(e.id) AS id
                    FROM employees e
                    LEFT JOIN organize_teams o ON e.team_id = o.id
                    WHERE e.is_leader = 1 AND o.is_deleted = 0 AND e.is_deleted = 0
                    GROUP BY e.team_id, o.team_name
                    LIMIT 0, 1000) AS Q2 ON Q1.team_id = Q2.team_id;
            """;
